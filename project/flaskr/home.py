...
def fetch_user_group_data(user_id,limit=10):
    ...
    if limit != None:
        upcoming_sessions_fetch = cursor.execute("SELECT Sessions.SessionID,Name,Date,StartTime,EndTime,Location,Sessions.GroupID FROM Sessions INNER JOIN Members ON Members.GroupID = Sessions.GroupID WHERE Members.UserID = ? AND Date > CURRENT_DATE OR (Date > CURRENT_DATE AND EndTime > CURRENT_TIME) ORDER BY Date ASC,StartTime ASC,EndTime ASC LIMIT ?",[user_id,limit]).fetchall()
    else:
        upcoming_sessions_fetch = cursor.execute("SELECT Sessions.SessionID,Name,Date,StartTime,EndTime,Location,Sessions.GroupID FROM Sessions INNER JOIN Members ON Members.GroupID = Sessions.GroupID WHERE Members.UserID = ? AND Date > CURRENT_DATE OR (Date > CURRENT_DATE AND EndTime > CURRENT_TIME) ORDER BY Date ASC,StartTime ASC,EndTime ASC",[user_id]).fetchall()
...
def fetch_user_overview_data(user_id):
    ...
    upcoming_sessions_fetch = cursor.execute("SELECT Sessions.SessionID,Name,Date,StartTime,EndTime,Location,Sessions.GroupID FROM Sessions INNER JOIN Members ON Members.GroupID = Sessions.GroupID WHERE Members.UserID = ? AND Date > CURRENT_DATE OR (Date > CURRENT_DATE AND EndTime > CURRENT_TIME) ORDER BY Date ASC,StartTime ASC,EndTime ASC",[user_id]).fetchall()
    for session_data in upcoming_sessions_fetch:
        count = cursor.execute("SELECT COUNT(UserID) FROM Attendees WHERE SessionID = ?",[joined_group_data[0]]).fetchone()[0]
